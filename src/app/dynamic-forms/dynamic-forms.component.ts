                  ...
                  if (element.fieldname == key.toLowerCase()) {
                    if (element.fieldtype == "datepicker") {
                      ...
                      element.fields.forEach(elementGroup => {
                        this.bindIdData["property"][key].forEach(elementGroupValue => {
                          elementGroup.value = elementGroupValue[elementGroup.fieldname];
                        });
                      });
                    } else {
              ...
              if (data) {
                if (data.length !== 0) {
                  ...
                  data.forEach((ele) => {
                    ...
                    if(ele && ele.property && ele.property.skillcategory) {

                      var skillObj = element.formfieldfilterValue.find(p=>p.name == ele.property.skillcategory);
                      if(skillObj) {
                        skillObj.pokemon.push({value: ele._id, viewValue: ele?.property?.title})
                      } else {
                        let obj = {
                          name: ele.property.skillcategory,
                          pokemon: [
                            {value: ele._id, viewValue: ele?.property?.title}
                          ]
                        }
                        element.formfieldfilterValue.push(obj);
                      }
                  ...
                  console.log("element", element.formfieldfilterValue)
                }
              }
            }, (err) => {
              console.error("err", err);
